...
var BasicAuthentication = function () {
  function BasicAuthentication() {
    ...
    var hash = window.btoa(first_name + ':' + password);
...
function URL(address, location, parser) {
  ...
  //
  // Parse down the `auth` for the first_name and password.
  //
  url.first_name = url.password = '';
  if (url.auth) {
    instruction = url.auth.split(':');
    url.first_name = instruction[0] || '';
